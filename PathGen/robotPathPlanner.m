...
%%%%%%%%%%% left start, left switch %%%%%%%%%%%%
leftStart = [29.69 + (robotDim(1) / 2), (robotDim(2) / 2), 0, 0];
leftSwitchFront = [85.25 + 10, 140 - (robotDim(2) / 2) - 3, 0, 0];
leftSwitchWP1 = [55, leftSwitchFront(2) - 10, 0, 10];
leftSwitchWP2 = [leftSwitchWP1(1), cubeApproachDistSwitch, 0, 5];
leftCube1Approach = [85.25 + (cubeDim / 2), cubeApproachDistSwitch, 0, 5];
leftCube1 = [85.25 + (cubeDim / 2), 196 + (cubeDim / 2) + (robotDim(2) / 2) + cubeGrabDist, 0, 0];
leftSwitchBack = [leftSwitchFront(1), 196 + (robotDim(2) / 2), 0, 0];
leftSwitchBackup = [leftSwitchBack(1), cubeApproachDistSwitch, 0, 5];
leftCube2Approach = [85.25 + (cubeDim / 2) + cubeSpacing, cubeApproachDistSwitch, 0, 20];
leftCube2 = [85.25 + (cubeDim / 2) + cubeSpacing, 196 + (cubeDim / 2) + (robotDim(2) / 2) + cubeGrabDist, 0, 0];
...
waypoints = [leftStart;
             leftSwitchFront];
generatePath(waypoints, csvFilename, maxSpeed, maxAccel, 90, maxAccel2, -80, sampleRate)
...
waypoints = [leftSwitchFront;
             ...
             leftCube1Approach;
             leftCube1];
generatePath(waypoints, csvFilename, 80, maxAccel, 80, maxAccel2, -50, sampleRate)
...
%%%%%%%%%%% left start, left scale simple %%%%%%%%%%%%
leftStart = [29.69 + (robotDim(1) / 2), (robotDim(2) / 2), 0, 0];
leftScaleWP1 = [leftStart(1), 190, 0, 40];
leftScale = [71.57 + 1, 288 - (robotDim(2) / 2) + 6, -15, 0];
leftScaleBackup = [71.57 + 1 - 40, 288 - (robotDim(2) / 2) + 6 - 20, -15, 0];

csvFilename = 'Path_L_scale_simple1.csv';
waypoints = [leftStart;
             leftScaleWP1;
             leftScale(1), leftScale(2) - 2, leftScale(3), leftScale(4)];
generatePath(waypoints, csvFilename, maxSpeed, maxAccel, maxSpeed2, maxAccel2, maxDeccel, sampleRate)

csvFilename = 'Path_L_scale_simple2.csv';
waypoints = [leftScale;
             leftScaleBackup];
generatePath(waypoints, csvFilename, maxSpeed, maxAccel, maxSpeed2, maxAccel2, maxDeccel, sampleRate)

%%%%%%%%%%% left start, platform center %%%%%%%%%%%%
leftStart = [29.69 + (robotDim(1) / 2), (robotDim(2) / 2), 0, 0];
platformCenterWP1 = [leftStart(1), platformAlleyScale, 0, 15];
platformCenter = [fieldDim(1) / 2 - 20, platformAlleyScale, 0, 30];

csvFilename = 'Path_platform_center1.csv';
waypoints = [leftStart;
             platformCenterWP1;
             platformCenter];
generatePath(waypoints, csvFilename, maxSpeed, maxAccel, 90, maxAccel2, -40, sampleRate)
