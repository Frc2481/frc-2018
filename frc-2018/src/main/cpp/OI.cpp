...
//operator
	m_aOpButton = new frc::JoystickButton(m_operatorStick, XB_A_BUTTON);
	m_bOpButton = new frc::JoystickButton(m_operatorStick, XB_B_BUTTON);
	m_xOpButton = new frc::JoystickButton(m_operatorStick, XB_X_BUTTON);
	m_yOpButton = new frc::JoystickButton(m_operatorStick, XB_Y_BUTTON);

	m_leftOpBumper = new frc::JoystickButton(m_operatorStick, XB_LEFT_BUMPER);
	m_rightOpBumper = new frc::JoystickButton(m_operatorStick, XB_RIGHT_BUMPER);

	m_startButton = new frc::JoystickButton(m_operatorStick, XB_START_BUTTON);
	m_backButton = new frc::JoystickButton(m_operatorStick, XB_BACK_BUTTON);
...
//driver
	m_shifterButton = new frc::JoystickButton(m_driverStick, XB_RIGHT_BUMPER);
	m_shifterButton->WhileHeld(new DriveTrainShiftCommand());

	m_fieldCentricButton = new frc::JoystickButton(m_driverStick, XB_LEFT_BUMPER);
...
//	m_climberButton->ToggleWhenPressed(new CLimberRetractCommand()); //figure out to toggle
	...
	m_armToIntakeFront = new frc::JoystickButton(m_driverStick, XB_A_BUTTON);
	m_armToIntakeFront->WhenPressed(new ArmIntakeFrontPosToggleCommand());

	m_armToIntakeBack = new frc::JoystickButton(m_driverStick, XB_X_BUTTON);
	m_armToIntakeBack->WhenPressed(new ArmIntakeBackPosToggleCommand());

	m_stowButtonDriver = new frc::JoystickButton(m_driverStick, XB_Y_BUTTON);
...
//	m_releaseFastCubeButton->WhenReleased(new IntakeRollerOffCommand());
	...
	m_intake3PosButton = new frc::JoystickButton(m_driverStick, XB_B_BUTTON);
	m_intake3PosButton->WhenPressed(new ArmToIntakePos3Command());

	m_driveTrainZeroGyro = new frc::JoystickButton(m_driverStick, XB_START_BUTTON);
...
//	m_intakeButton->WhenReleased(new IntakeStopCubeCommandGroup());
	...
	m_mirrorArmPos = new frc::JoystickButton(m_operatorStick, XB_RIGHT_BUMPER);
	m_mirrorArmPos->WhenPressed(new ArmPosMirrorCommand());

	m_stowButtonOp = new frc::JoystickButton(m_operatorStick, XB_LEFT_BUMPER);
