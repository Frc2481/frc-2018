...
//		frc::SmartDashboard::PutData("Drive Train Test", new DriveTrainTestCommandGroup());
		...
		m_posChooser->SetDefaultOption("Left", POS_LEFT);
		m_posChooser->AddOption("Center", POS_CENTER);
		m_posChooser->AddOption("Right", POS_RIGHT);

		m_crossCenterChooser = new SendableChooser<Autos>();
		m_crossCenterChooser->SetDefaultOption("Can cross center", CAN_CROSS);
		m_crossCenterChooser->AddOption("Can't cross center", CANT_CROSS);
