...
CTREMagEncoder::CTREMagEncoder(TalonSRX* talon, const std::string& name)
	...
	m_offset = Rotation2D::fromDegrees(frc::Preferences::GetInstance()->GetDouble(m_calibrationKey));
...
void CTREMagEncoder::Calibrate() {
	...
	frc::Preferences::GetInstance()->PutDouble(m_calibrationKey, m_offset.getDegrees());
}
