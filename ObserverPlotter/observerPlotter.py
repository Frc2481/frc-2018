...
filename = "ObserverLog_tornado.csv"
...
def updatePoint(n, x, y, heading, times, start, point, robot, timeTracker):
    point.set_data(np.array([x[n], y[n]]))
    robotData = genRobotSquare((x[n], y[n]), heading[n])
    robot.set_data([p[0] for p in robotData], [p[1] for p in robotData])
    timeTracker.set_text("Time: " + "{0:.2f}".format((times[n]-start)/1000000.0) + "sec")
    return [point, robot, timeTracker]
...
#Calculate the average step between lines to we can calculate a frame rate
gaps = []
...
#Animate
ani = animation.FuncAnimation(fig, updatePoint, len(xdata), fargs=(xdata, ydata, headings, ts, startTime, point, robot, time), interval=avgIntervalMs)
